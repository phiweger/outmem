...
from outmem.cli._common import TIMESTAMP_FMT as _TIMESTAMP_FMT
from outmem.cli._common import agent_identity as _agent_identity
from outmem.cli._common import base_root
from outmem.cli._common import status as _status
from outmem.cli.repo import register as register_repo_commands
from outmem.config import SEMANTIC_INDEX_PAGES, SEMANTIC_UNAVAILABLE_HELP
from outmem.exceptions import OutmemError
from outmem.lint import LintReport
from outmem.repo import REGISTRY_FILENAME, load_registry, split_subject
from outmem.sources import SOURCES_DIR, SOURCES_LOCAL_DIR
from outmem.store import WikiStore


def _resolve_root(args: argparse.Namespace) -> Path:
    """The wiki a subcommand acts on — ``base_root``, or a wiki inside it."""
    base = base_root(args)
...
def _lint_exit_code(report: LintReport, *, error_only: bool) -> int:
    """``2`` on any error, ``1`` on warnings unless ``--error-only``, else ``0``."""
    if report.has_errors:
        return 2
    if report.has_findings and not error_only:
        return 1
    return 0


def cmd_lint(args: argparse.Namespace) -> int:
    from outmem.lint import format_report, indexed_paths_or_none, lint_repository, lint_wiki

    if getattr(args, "repo", False):
        # The registry plus every wiki in it, as one report whose paths
        # are repo-relative — `wikis/legal/wiki/pages/nda.md` says which
        # wiki without a header per wiki.
        report = lint_repository(base_root(args))
    else:
        store = _open_store(args)
        report = lint_wiki(
            store.wiki_path,
    ...
    return _lint_exit_code(report, error_only=args.error_only)
