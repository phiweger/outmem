...
Logfire when ``config.yaml`` sets ``logfire.enabled: true``, but
...
themselves, so wikis with Logfire enabled get instrumentation from
every code path. For custom integrations, call this once at startup::
