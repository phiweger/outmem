...
**Source identity and supersession (v0.7).** The content-addressed path is what makes re-ingesting the same bytes a no-op, but it is also why a *revised* document is invisible as a revision: new content means a new hash, a new path, and therefore a new row unrelated to the one it replaces. `document_key` is the identity that survives a revision — the answer to "which document is this a version of" — and `superseded_by` is the edge between consecutive versions. Nothing is deleted when a document is superseded: the old file, its sha and its ingestion history are precisely what makes the edge useful.

Identity is **declared, not inferred** (`outmem ingest --as`). outmem derives it from the path when nothing else claims the derived name, and **refuses the ingest** when something does. The two readings of a collision — "a new version of that document" and "a different document that happens to share a filename" — are indistinguishable from the *wiki* path, because copying into `<into>/<sha>/<filename>` has already discarded what told them apart, and the collision is the norm rather than the exception for a pipeline emitting `<drug>/output/<hash>/document.md`. Guessing wrong would write a supersession edge that later drives a recheck of one drug's page against another drug's source, so outmem asks. `origin_path` is recorded for the same reason and closes the loop: it is the one field in which the distinguishing part survives, so the refusal quotes both origins and proposes a name from the first segment where they diverge.

**A key names a document, not a file.** Derived keys are case-folded and have the source extension stripped, because both are properties of the manifestation rather than of the work: a pipeline that starts exporting `.txt` where it exported `.md` must keep the same identity, since the alternative is a *silent* new identity — the failure mode this whole mechanism exists to remove. The normalisation makes collisions more likely, which is safe in the direction that matters: a collision is a refusal, never a merge. Keys are otherwise free-form, so a real external identifier (`awmf/113-001`, `doi/…`) is a legal and preferable key wherever one exists. They are separated by `/` rather than the `:` of page slugs (§4), keeping the two identifier namespaces visually disjoint where both appear in the same output.

**One live row per identity, enforced where the write happens.** A document key with two un-superseded rows is a broken index: `latest_for` picks one head, the other stays live forever, and a page compacted from it is never reported as stale. Every path that writes an identity — a fresh ingest, `--as` on already-registered content, and `sources backfill` — resolves the claim inside a single `BEGIN IMMEDIATE` transaction *reading the database*, not the snapshot the process loaded at open. Deciding it from the snapshot is unsound precisely because §7 blesses parallel ingest: two concurrent runs would each see v1 as the live head and each supersede it.

**Frozen sources naming page slugs are uncoupled by a recorded mapping.** The reverse hazard to supersession: a content-addressed file can never change, while a page slug moves on every reorganisation, so a source that names slugs holds a reference that rots. Rewriting the source is not an option — a source is a faithful copy of what was handed over, and rewriting changes its sha and therefore its identity. So `source_refs` records, at ingest, which page each named slug resolved to, and `rename_page` re-points that mapping. The reference is then held by identity rather than by a string, and survives a reorganisation even with `--no-alias`. An `[[wikilink]]` in a source is an *exact* reference; a bare prose token is a heuristic one, gated on an existing namespace, because the slug grammar cannot distinguish a single-segment slug from a word or `12:30` from a time. Ingest warns rather than refuses — it is the operator's document, and ingest is the last moment it is editable. This protects against renames, not deletions: a deleted page leaves the mapping pointing at nothing, which makes the break precise but does not repair it.

**Provenance becomes a liveness signal.** Because a page's `provenance:` names the source *versions* it was compacted from, and supersession says which versions are no longer current, the intersection is computable: the pages that may no longer hold. `outmem stale` reports it, following the chain to the newest version. It reports only — whether a page still stands is a judgement call, and on clinical material that belongs to a human or to an explicit agent run over the list, never to a side effect of ingest. The exit from the report is `extend_page(provenance=…)`, which re-compacts the page and re-points the citation in one commit; without it a reported page would keep citing the superseded version and keep being reported.
